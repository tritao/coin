...
struct Options {
  std::string spec_dir = "tests/render/specs";
...
bool ensure_parent(const std::string& path) {
  return ensure_directory(parent_directory(path));
}
...
std::string computeArtifactsBase() {
  std::string dir = getExecutableDirectory();
  return dir + "/../render_artifacts";
}
...
void print_usage() {
  std::cerr << "Usage: CoinRenderTests run [--spec-dir <dir>] [--backend <gl|gles|web>]"
            << " [--artifacts-dir <dir>] [--only id1,id2] [--update-baselines] [--retries N]\n";
  std::cerr << "Defaults: spec-dir=tests/render/specs backend=gl artifacts-dir=bld/render_artifacts/gl\n";
}
...
OutputResult run_single_output(const std::string& spec_path,
  ...
  if (update_baselines) {
    if (invoke_snapshot(spec_path, opts.backend, actual) != 0) {
...
int run_spec(const SpecEntry& entry, const Options& opts, bool update_baselines, int retries) {
  ...
  auto print_result_summary = [&](const OutputResult& result) {
    ...
    std::cout << "  " << status << label << kColorReset << "  (" << stats.str() << ")\n"
...
int run_with_options(int argc, const char* argv[]) {
  ...
  if (opts.artifacts_dir.empty()) {
    opts.artifacts_dir = computeArtifactsBase();
  }
  ...
  for (const auto& entry : specs) {
    if (run_spec(entry, opts, opts.update_baselines, opts.retries) != 0) {
      failed = true;
