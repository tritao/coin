...
public:
  ...
  struct TextureCopyKey {
    ...
    bool operator==(const TextureCopyKey & other) const
    {
      return this->source == other.source &&
             this->byteCount == other.byteCount;
    }
  };
  struct TextureCopyKeyHash {
    size_t operator()(const TextureCopyKey & key) const
    {
      const size_t pointerHash = std::hash<const void *>()(key.source);
      return pointerHash ^ (key.byteCount + (pointerHash << 6) +
                            (pointerHash >> 2));
    }
  };
...
const unsigned char *
SoIRRenderAction::retainTextureData(const unsigned char * bytes,
                                    size_t byteCount)
{
  if (!bytes || byteCount == 0) return NULL;

  SoIRRenderActionP::TextureCopyKey key = { bytes, byteCount };
  const auto found = PRIVATE(this)->textureCopies.find(key);
  if (found != PRIVATE(this)->textureCopies.end()) return found->second;

  unsigned char * copy = static_cast<unsigned char *>(
    PRIVATE(this)->geometryPool.allocate(byteCount, alignof(unsigned char)));
  std::memcpy(copy, bytes, byteCount);
  PRIVATE(this)->textureCopies.emplace(key, copy);
  return copy;
}
