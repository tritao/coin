                                  ...
                                  const SoRenderCommand & cmd)
{
  ...
  if (this->posLoc >= 0 && entry.posVBO) {
    cc_glglue_glBindBuffer(this->glue, GL_ARRAY_BUFFER, entry.posVBO);
    this->glue->glEnableVertexAttribArray(this->posLoc);
    this->glue->glVertexAttribPointer(this->posLoc, 3, GL_FLOAT, GL_FALSE, stride, nullptr);
  }
  ...
  if (this->normLoc >= 0) {
    if (entry.normVBO) {
      cc_glglue_glBindBuffer(this->glue, GL_ARRAY_BUFFER, entry.normVBO);
      this->glue->glEnableVertexAttribArray(this->normLoc);
      this->glue->glVertexAttribPointer(this->normLoc, 3, GL_FLOAT, GL_FALSE, stride, nullptr);
    }
    else {
      this->glue->glDisableVertexAttribArray(this->normLoc);
  ...
  if (this->colorLoc >= 0) {
    if (entry.colorVBO) {
      cc_glglue_glBindBuffer(this->glue, GL_ARRAY_BUFFER, entry.colorVBO);
      this->glue->glEnableVertexAttribArray(this->colorLoc);
      this->glue->glVertexAttribPointer(this->colorLoc, 4, GL_FLOAT, GL_FALSE, 0, nullptr);
    }
    else {
      this->glue->glDisableVertexAttribArray(this->colorLoc);
      this->glue->glVertexAttrib4f(this->colorLoc, 1.0f, 1.0f, 1.0f, 1.0f);
    }
  }
  ...
  if (this->lineDistLoc >= 0) {
    if (entry.lineDistVBO) {
      cc_glglue_glBindBuffer(this->glue, GL_ARRAY_BUFFER, entry.lineDistVBO);
      this->glue->glEnableVertexAttribArray(this->lineDistLoc);
      this->glue->glVertexAttribPointer(this->lineDistLoc, 1, GL_FLOAT, GL_FALSE, 0, nullptr);
    }
    else {
      this->glue->glDisableVertexAttribArray(this->lineDistLoc);
  ...
  if (this->texcoordLoc >= 0) {
    if (entry.texcoordVBO) {
      cc_glglue_glBindBuffer(this->glue, GL_ARRAY_BUFFER, entry.texcoordVBO);
      this->glue->glEnableVertexAttribArray(this->texcoordLoc);
      this->glue->glVertexAttribPointer(this->texcoordLoc, 2, GL_FLOAT, GL_FALSE, 0, nullptr);
    }
    else {
      this->glue->glDisableVertexAttribArray(this->texcoordLoc);
      this->glue->glVertexAttrib2f(this->texcoordLoc, 0.0f, 0.0f);
    }
  }
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (hasSelection) {
      ...
      if (cmd.selection.selectWholeObject && prim == GL_TRIANGLES
        ...
        this->glue->glEnableVertexAttribArray(this->posLoc);
        this->glue->glVertexAttribPointer(this->posLoc, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
        if (this->normLoc >= 0) {
          this->glue->glDisableVertexAttribArray(this->normLoc);
          this->glue->glVertexAttrib3f(this->normLoc, 0.0f, 0.0f, 1.0f);
        }
